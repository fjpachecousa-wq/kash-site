...
function FormWizard({ open, onClose }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      try { saveTrackingShortcut(code); await apiUpsertFull({ kashId: code, companyName: form.company?.companyName || "", ...payload, faseAtual: "Recebido", subFase: "Formulário", statusNota: "Contrato disponível", source: "kashsolutions.us", updates: [{ ts: dateISO, status: "Formulário recebido", note: "Contrato disponível", faseAtual: "Recebido", subFase: "Formulário" }] }); await apiUpdate({ kashId: code, faseAtual: 1, subFase: null, status: 'Formulário recebido', note: 'Contrato criado' }); } catch(e) { console.warn('API falhou', e); }
