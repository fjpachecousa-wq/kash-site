...
/* KASH patch — NÃO REMOVE NADA, só adiciona runtime para Sheets e bloqueio Formspree */
(function(){
  try{
    const ENDPOINT = (window.PROCESSO_API_URL || window.PROCESSO_API || "https://script.google.com/macros/s/AKfycby9mHoyfTP0QfaBgJdbEHmxO2rVDViOJZuXaD8hld2cO7VCRXLMsN2AmYg7A-wNP0abGA/exec");

    function getTracking(){
      try{
        const v = (localStorage.getItem("last_tracking") || localStorage.getItem("kash_last_tracking") || "").toUpperCase();
        return v && v.startsWith("KASH-") ? v : v;
      }catch(_){ return ""; }
    }
    function getCompany(){
      try{ return (document.querySelector('input[name="companyName"]')?.value || "").trim(); }catch(_){ return ""; }
    }
    ...
    // Neutraliza qualquer fetch ao Formspree sem remover blocos da UI
    const _fetch = window.fetch;
    window.fetch = function(input, init){
      try{
        const url = (typeof input==="string") ? input : (input && input.url) || "";
        if (/formspree/i.test(url)) { return Promise.resolve(new Response(null,{status:204})); }
      }catch(_){}
      return _fetch.apply(this, arguments);
    };

    // Anexa o handler ao botão "Concluir (teste)" SEM mexer no JSX
    function attach(){
      const btns = Array.from(document.querySelectorAll("button, a, [role='button']"));
      for(const b of btns){
        const txt = (b.textContent || "").trim().toLowerCase();
        if (txt.includes("concluir") && txt.includes("teste")){
          if (!b.__kashBound) {
            b.__kashBound = true;
            b.addEventListener("click", async (ev)=>{
              try{ ev.preventDefault(); }catch(_){}
              await send({});
              try{ window.location.href="/success.html"; }catch(_){}
            });
          }
        }
      }
