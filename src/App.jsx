...
export default function App(){ 
  ...
  function onChange(e, idx){ const {name,value}=e.target; if(name.startsWith("socio.")){ const [,f]=name.split("."); const c=[...form.socios]; c[idx][f]=value; setForm({...form, socios:c}); } else { setForm({...form, [name]: value}); if(name==="companyName") localStorage.setItem("companyName", value); } }
  function addSocio(){ setForm({...form, socios:[...form.socios, {nome:"",doc:""}]}) }
  async function gerarTracking(){ const id=newKashId(); setKashId(id); localStorage.setItem("kashId", id); try{ if(APPS_SCRIPT_URL) await postJSON(APPS_SCRIPT_URL, { action:"upsert", kashId:id, companyName: form.companyName||localStorage.getItem("companyName")||"", atualizadoEm:new Date().toISOString(), status:"Rascunho" }); }catch{} setStep(2); }
  function contratoHTML(){ const dataBR=new Date().toLocaleString("pt-BR"); const sociosList=(form.socios||[]).map((s,i)=>`<li>${i+1}. ${s.nome} — ${s.doc||""}</li>`).join(""); return `<style>body{font-family:Arial,sans-serif;margin:32px;line-height:1.5}.muted{color:#666}.box{border:1px solid #ddd;padding:16px;border-radius:8px}.sign{margin-top:48px}.sign .line{margin:28px 0 40px;border-bottom:1px solid #000;height:0}.footer{position:fixed;bottom:16px;left:32px;right:32px;font-size:12px;color:#666;display:flex;justify-content:space-between}</style><h1>Contrato de Prestação de Serviços</h1><div class="muted">KASH Corporate Solutions LLC — Tracking: ${kashId}</div><div class="box"><h3>Objeto</h3><p>Registro na Flórida (Sunbiz), posterior EIN, endereço e agente por 12 meses.</p><h3>Dados</h3><p><b>Empresa:</b> ${form.companyName||"(informar)"}<br/><b>Responsável:</b> ${form.clienteNome||"(informar)"} — <b>E-mail:</b> ${form.clienteEmail||"(informar)"}</p><p><b>Sócios</b></p><ul>${sociosList||"<li>(informar)</li>"}</ul><h3>Pagamento</h3><p>Vigência após pagamento (ref. US$ 1.360).</p><h3>Foro</h3><p>RJ — Capital; se necessário, Orlando — Flórida.</p></div><div class="sign"><p>Local e Data: ________________________________ — ${dataBR}</p><div class="line"></div><p>Assinatura</p></div><div class="footer"><span>${form.companyName||""}</span><span>Tracking: ${kashId}</span></div>`; }
  function imprimirContrato(){ const w=window.open("","_blank"); w.document.write(contratoHTML()); w.document.close(); w.focus(); w.print(); }
  async function pagar(){ if(!STRIPE_PAYLINK){alert("Defina STRIPE PAYLINK");return} try{ if(APPS_SCRIPT_URL) await postJSON(APPS_SCRIPT_URL, { action:"event", kashId, companyName: form.companyName||localStorage.getItem("companyName")||"", status:"Pagamento iniciado", atualizadoEm:new Date().toISOString(), event:"payment_start" }); }catch{} window.location.href=STRIPE_PAYLINK; }
  async function concluir(){ setLoading(true); setError(""); try{ if(!APPS_SCRIPT_URL) throw new Error("Configure APPS_SCRIPT_URL"); const res=await postJSON(APPS_SCRIPT_URL, { action:"upsert", kashId, companyName: form.companyName||localStorage.getItem("companyName")||"", clienteNome: form.clienteNome, clienteEmail: form.clienteEmail, socios: form.socios, atualizadoEm:new Date().toISOString(), status:"Concluído" }); if(!res||res.ok===false) throw new Error(res&&res.error?res.error:"Falha ao salvar"); setStep(6); } catch(e){ setError(e.message||"Erro ao salvar"); } finally { setLoading(false); } }
  async function consultarStatus(){ setLoading(true); setError(""); setStatusData(null); try{ if(!APPS_SCRIPT_URL) throw new Error("Configure APPS_SCRIPT_URL"); const res=await postJSON(APPS_SCRIPT_URL, { action:"status", kashId }); setStatusData(res); } catch(e){ setError(e.message||"Erro ao consultar"); } finally { setLoading(false); } }
    ...
    <div className="relative flex items-center justify-center">
      <div style={ maxWidth: 960, margin: "0 auto", padding: 16 }>
        ...
        {step===1&&(<div><h2>1) Dados</h2>
          ...
          <div style={marginTop:16}><button onClick={gerarTracking} style={B}>Gerar Tracking (kashId)</button></div>
        </div>)}

        {step===2&&(<div><h2>2) kashId</h2>
        ...
        {step===6&&(<div><h2>6) Acompanhar</h2>
          <div style={display:"flex",gap:8}>
            ...
            <button onClick={consultarStatus} style={B}>Consultar</button>
          </div>
