...
// Variáveis de ambiente (Vite) com fallback para window.*
const APPS_SCRIPT_URL =
  (typeof import.meta !== "undefined" && import.meta.env?.VITE_APPS_SCRIPT_URL) ||
  (typeof window !== "undefined" && window.APPS_SCRIPT_URL) ||
  "";

const STRIPE_PAYLINK =
  (typeof import.meta !== "undefined" && import.meta.env?.VITE_STRIPE_PAYLINK) ||
  (typeof window !== "undefined" && window.STRIPE_PAYLINK) ||
  "";
...
export default function App() {
  ...
  function onChange(e, idx) {
    ...
    if (name.startsWith("socio.")) {
      const [, f] = name.split(".");
      const c = [...form.socios];
      c[idx][f] = value;
      setForm({ ...form, socios: c });
    } else {
      setForm({ ...form, [name]: value });
      if (name === "companyName") localStorage.setItem("companyName", value);
    }
  }
  function addSocio() {
    setForm({ ...form, socios: [...form.socios, { nome: "", doc: "" }] });
  }
  function next() {
    setStep((s) => s + 1);
  }
  ...
  async function gerar() {
    const id = newKashId();
    setKashId(id);
    localStorage.setItem("kashId", id);
    try {
      if (APPS_SCRIPT_URL) {
  ...
  function contratoHTML() {
    ...
    const socios = (form.socios || [])
      .map((s, i) => `<li>${i + 1}. ${s.nome} — ${s.doc || ""}</li>`)
      .join("");
  ...
  async function concluir() {
    ...
    try {
      ...
      const res = await postJSON(APPS_SCRIPT_URL, {
        action: "upsert",
        kashId,
        companyName:
          form.companyName || localStorage.getItem("companyName") || "",
  ...
  async function consultar() {
    ...
    try {
      if (!APPS_SCRIPT_URL) throw new Error("Configure APPS_SCRIPT_URL");
      const r = await postJSON(APPS_SCRIPT_URL, { action: "status", kashId });
      setStatusData(r);
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 16 }}>
      ...
      <p style={{ marginTop: 0, color: "#555" }}>
        Fluxo: Coleta → Tracking → Contrato → Pagamento → Gravar → Status
      </p>
      ...
      {step === 1 && (
        <div>
          ...
          <div style={{ marginTop: 8 }}>
            <b>Sócios</b>{" "}
            <button type="button" onClick={addSocio} style={BG}>
              + Adicionar
            </button>
          </div>
          {(form.socios || []).map((s, i) => (
          ...
          <div style={{ marginTop: 16 }}>
            <button onClick={gerar} style={B}>
              Gerar Tracking (kashId)
            </button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          ...
          <button onClick={() => setStep(3)} style={B}>
            Avançar
          </button>
        </div>
      )}
      ...
      {step === 3 && (
        <div>
          <h2>3) Contrato</h2>
          <button onClick={imprimirContrato} style={B}>
            Visualizar/Imprimir Contrato
          </button>{" "}
          <button onClick={() => setStep(4)} style={BG}>
            Continuar
          </button>
        </div>
      )}
      ...
      {step === 4 && (
        <div>
          <h2>4) Pagamento</h2>
          <button onClick={() => setStep(3)} style={BG}>
            Voltar
          </button>{" "}
          <button onClick={pagar} style={B}>
            Pagar
          </button>{" "}
          <button onClick={() => setStep(5)} style={BG}>
            Já paguei
          </button>
        </div>
      )}
      ...
      {step === 5 && (
        <div>
          ...
          <button onClick={concluir} style={B} disabled={loading}>
      ...
      {step === 6 && (
        <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              value={kashId}
              onChange={(e) => setKashId(e.target.value)}
              style={I}
            />
            <button onClick={consultar} style={B}>
              Consultar
            </button>
          </div>
          {statusData && (
